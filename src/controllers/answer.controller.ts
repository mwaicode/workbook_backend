...
export const addAnnotation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    io.emit('annotation:added', { answerId: req.params.id })
...
export const updateAnnotation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    io.emit('annotation:added', { answerId: annotation.answerId })
...
//     io.emit('grade:updated', { answerId: req.params.id, studentId: answer.studentId })
...
export const submitGrade = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const answer = await prisma.answer.findUnique({
      where: { id: req.params.id },
      include: { question: { select: { worksheetId: true } } }  // ← add worksheetId
    })
    if (!answer) { res.status(404).json({ error: 'Answer not found' }); return }
    const grade = await prisma.grade.upsert({
      where: { answerId: req.params.id },
    ...
    const payload = {
      ...
      worksheetId: answer.question.worksheetId  
    }
    console.log('📡 Emitting grade:updated', payload)  
    io.emit('grade:updated', payload)

    res.json(grade)
  } catch (e) {
    console.error('submitGrade error:', e)  
...
export const reviewGrade = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    io.emit('grade:updated', { answerId: grade.answerId, studentId: grade.answer.student.id })
